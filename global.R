...
fdata <<- read.csv("./fertility_data_raw.csv", header = TRUE, stringsAsFactors = FALSE)
states <<-read.csv("./states.csv", header = TRUE, stringsAsFactors=FALSE)
##clean data and transform to appropriate class

# ##state list State
fdata$State <- as.factor(fdata$State)
# 
